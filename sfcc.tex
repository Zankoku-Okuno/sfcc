...
%Even worse, we may lose the ability to easily map from the static source code to the dynamic control flow of a program.
...
TODO: meta-continuatinos as in MFDC. I know there's some research in implementing continuations effectively in Racket (which I may have around here somewhere).
	...
	%
	%\section{Impure Static Semantics}
	%\label{impureStatics}
	%
	%Once we get beyond the simple uses of first-class control, it can be difficult to understand what types of values a continuation expects to receive and what types it produces.\footnote{In fact, although it is fairly easy to work out the types given a continuation, it may be difficult to work out from the source code which particular continuation there will be at runtime. The exact continuation captured may easily be dependent on user input, so one answer may be insufficient, which to this author is a very interesting property of first-class control.}
	%As such, we would like a static type system to help us identify type errors in our code.
	%We will begin by developing an impure type system, since it takes advantage of only the most basic features of our base calculus and is thus easiest to understand.
	%We will make use of two judgements in particular, the is-of-kind judgement, written $\hasKind{\Delta}{\sigma}{\kappa}$ and the is-of-type judgement $\hasType{\Delta}{\Gamma}{e}{\sigma}$.
	%We extend the type system with the axioms given in Figure~\ref{fig:impureStatics}, which we will now examine in plain language.
	%
	%\begin{figure}
	%\caption{Static Semantics for Impure Control}
	%\label{fig:impureStatics}
	%
	%\begin{center}\fbox{$\hasKind{\Delta}{\sigma}{\kappa}$}\end{center}
	%
	%	\begin{prooftree}
	%	\AxiomC{}
	%	\UnaryInfC{$\hasKind{\Delta}{\Cue}{\type\to\type\to\type}$}
	%	\end{prooftree}
	%
	%	\begin{prooftree}
	%	\AxiomC{}
	%	\UnaryInfC{$\hasKind{\Delta}{\Subcont}{\type\to\type\to\type}$}
	%	\end{prooftree}
	%	
	%\begin{center}\fbox{$\hasType{\Delta}{\Gamma}{e}{\sigma}$}\end{center}
	%	
	%	\begin{prooftree}
	%	\AxiomC{}
	%	\UnaryInfC{$\hasType{\Delta}{\Gamma}{\newhandler}{\Cue\;\sigma\;\sigma'}$}
	%	\end{prooftree}
	%
	%	\begin{prooftree}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e_p}{\Cue\;\sigma'\;\sigma}$}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e'}{\sigma'\to\sigma}$}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e}{\sigma}$}
	%	\TrinaryInfC{$\hasType{\Delta}{\Gamma}{\handle{e_p}{e'}{e}}{\sigma}$}
	%	\end{prooftree}
	%
	%	\begin{prooftree}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e_p}{\Cue\;\sigma'\;\sigma}$}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e}{\sigma'}$}
	%	\BinaryInfC{$\hasType{\Delta}{\Gamma}{\abort{e_p}{e}}{\sigma_{any}}$}
	%	\end{prooftree}
	%
	%	\begin{prooftree}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e_p}{\Cue\;\sigma''\;\sigma'}$}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e}{(\sigma''\to\sigma')\to\Subcont\;\sigma\;\sigma'\to\sigma}$}
	%	\BinaryInfC{$\hasType{\Delta}{\Gamma}{\capture{e_p}{e}}{\sigma}$}
	%	\end{prooftree}
	%
	%	\begin{prooftree}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e'}{\Subcont\;\sigma\;\sigma'}$}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e}{\sigma}$}
	%	\BinaryInfC{$\hasType{\Delta}{\Gamma}{\pushSubcont{e'}{e}}{\sigma'}$}
	%	\end{prooftree}
	%
	%	\begin{prooftree}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e_{in}}{\sigma'}$}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e}{\sigma}$}
	%	\BinaryInfC{$\hasType{\Delta}{\Gamma}{\onWind{e_{in}}{e}}{\sigma}$}
	%	\end{prooftree}
	%
	%	\begin{prooftree}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e_{out}}{\sigma'}$}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e}{\sigma}$}
	%	\BinaryInfC{$\hasType{\Delta}{\Gamma}{\onUnwind{e_{out}}{e}}{\sigma}$}
	%	\end{prooftree}
	%
	%	\begin{prooftree}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e_{out}}{\sigma'}$}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e}{\sigma}$}
	%	\BinaryInfC{$\hasType{\Delta}{\Gamma}{\onAbort{e_{out}}{e}}{\sigma}$}
	%	\end{prooftree}
	%
	%\end{figure}
	%
	%Perhaps the most important point in this type system is that the type of cues is in fact a type constructor of two arguments.
	%The first type argument, which we call the in-type, specifies the types of values that $\textsf{abort}$ is allowed to send to a stack mark identified by the cue.
	%The second type argument, which we call the out-type, is the type of values that the cue should allow to pass directly into the rest of the control stack, regardless of whether they are generated normally, or by the evaluation of the associated handler function.
	%
	%This is best seen in the axioms for $\textsf{handle}$ and $\textsf{abort}$.
	%Recall that there are two ways to exit from the extent of a \textsf{handle} -- normal and abnormal exit.
	%When we mark the stack using a \textsf{handle} expression, the type of value obtained must be the same whether from normal or abnormal exit.
	%Further, since values unwind past the stack mark without alteration, this type must also be the same as the type of the body.
	%Finally, the handler function provided must take the cue's in-type to its out-type, and this is really what determines how we think about the type parameters of cues.
	%The cue's in- and out-types provide constraints to ensure that each cue may only be found in certain type contexts on the stack.
	%We discuss the rest of the technique in our discussions of \textsf{abort} and \textsf{capture}.
	%
	%Now when we abort, we use the in-type of the cue to ensure that any found handler function will be able to accept the abort value.
	%The type produced by the handler function has no affect on the abort expression itself, so we need not specify any particular type, though the cues in scope may give the programmer some idea anyway.
	%Also, since execution will discard some top-most section of the stack, losing its type context, the type of an abort expression is unconstrained.
	%So, \textsf{abort} only need check that its value is acceptable to any handler function it might be sent to.
	%
	%We now turn to capturing continuations, starting with the \textbf{Subcont} type constructor.
	%This type constructor is the type for a continuation, and its type parameters determine what type the stack section expects to receive and what type it can then produce.
	%It is very much like the function type in this way, and in fact, the author had considered re-using the function type here, but again decided to be explicit with the types to better note the novel features of the FCC.
	%
	%Recall from the dynamic semantics that the evaluation of a \textsf{capture} expression (culminating in rule \textsc{c}) not only reifies a portion of the stack, but also passes the deliminting mark's handler function and the reified continuation to another function $e$.
	%The type of the handler function is determined by the type of the delimiting cue.
	%The result type of the function $e$ must be the same as the type expected in the context of the \textsf{capture} expression itself, since the function is immediately tail-called.
	%The type parameters of \textsf{Subcont} are fixed in a somewhat subtle way.
	%The captured subcontinuation extends from (at the bottom) just above the cue to (at the top) just below the \textsf{capture} expression itself.
	%Therefore, the \textbf{Subcont} expects to receive types of the same value as we expect to be produced by the \textsf{capture} expression.
	%We also tie the out-type of the cue to the type produced by the subcontinuation, as it is (recall the explanation for \textsf{handle}) the type that is expected just below the bottom of the subcontinuation.
	%
	%For the last control operator, \textsf{restore}, it suffices to check that the type of the value fed into the continuation is acceptable to that continuation, and to derive the result type from the type we expect from the continuation.
	%Again, this is essentially the same as typing a function application.
	%Thus, the typing rules for the four control operators conspire, ``coordinating'' through the parameters of the \textbf{Cue} type constructor, to ensure that the stack is only split apart or spliced back together in ways that respect the types of values that can unwind past the split/join points.
	%
	%Finally, the three rules for guard expressions are all quite uninteresting.
	%Since the wind expression is evaluated in its own sub-interpreter and its value discarded, the type of the guarding expression is unconstrained.
	%Then, evaluation resumes with the body, so the type of the entire guard expression is the same as the type of the body.
	%
	%TODO when we create a cue, we can restrict its type.
	%if the out-type is not restricted, then it is useless for continuation-capture, but we can install it in any context.
	%if the in-type is not restricted, I'm not sure it's useful at all.
	%constraining both types allows it to be used for continuations, and if we have them related by a single type variable
	%
	%
	%
	%%It looks like: when we abort, we only care about the input type of the handler. When we capture, we only care about the output type (unless we want to reinstate the handler?). Furthermore, when we install a handler, the output type of the handler specifically should be fixed (so we can install it in multiple contexts). Perhaps the thing to do is exploit the fact that capture can be translated as withSubcont+restore.
	%
	%%$\textbf{Prompt}\;\sigma$ defined as $\Cue\;\sigma\;\sigma$
	%%capture uses Handler to grab the subcontinuation, and yes it does grab the installed handling function
	%
	%
	%%Prompts are pushed onto the control stack as a single value, i.e. there is no additional data associated with the prompt.
	%%The prompt type constructor is parameterized by a single type, which is the type of values which may pass down\footnote{assuming an upward-growing stack} the stack through the prompt.
	%%It makes no difference if the values are passed because of normal evaluation or an abort.
	%
	%%In contrast, our basic stack marker, which we call a ``cue'', is associated with a handler function at the time when it is pushed to the control stack.
	%%Furthermore, its type constructor is parameterized by two types.
	%%We call the first an ``in-type'', which specifies the type of values which can be used to invoke a handler function associated with the cue.
	%%The second, which we call an ``out-type'' specifies the type of values which may pass down the stack through the cue, just as the in prompts.
	%
	%%From the first three rules, we get exceptions. Adding the next two allow us to use first-class control: it is easy to show that with the following definitions, we can recover the system in MFDC. The final three rules add winding protection capabilities.
	%
	%We had considered a formulation of $\onWind$ as a binding form, as in Python and C\# context managers.
	%However, this would break normal models of substitution.
	%Consider an $\onWind$ that is responsible for opening a file: each call to $\texttt{fopen}$ produces a potentially different file pointer.
	%Therefore, if the resultant file pointer were to always be bound to $x$, each time we restored a subcontinuation involving this $\onWind$, we would need to re-bind $x$ everywhere.
	%This is impossible with a substitution model of binding, as substitution erases variables as they are bound.
	%To avoid these awkward binding issues, guard expressions of the three \textsf{onX} forms should be responsible only for state changes, and have designed our operators appropriately.
	%We will see an example of file handle management as we explore applications.
	%We will also see that the apparent limitation of not being able to bind a variable is not a problem when simulating Python- and C\#-style context managers, as those languages' variable implicitly represent mutable cells.
	%
	%\section{Monadic Static Semantics}
	%\label{pureStatics}
	%
	%\subsection{Monad}
	%perhaps use a fixed \texttt{Obs} type here, or perhaps dump into the IO sin-bin
	%
	%this semantics is too restrictive: there are externally pure functions which use FCC internally, such as the foldl/ee in Figure~\ref{fig:ex-foldl/ee}. We therefore need a way to more accurately type expressions whose control effects are encapsulated.
	%
	%\begin{figure}
	%\caption{Static Semantics for Monadic Control}
	%\label{fig:monadicStatics}
	%
	%\begin{center}\fbox{$\hasKind{\Delta}{\sigma}{\kappa}$}\end{center}
	%
	%	\begin{prooftree}
	...
	%	\begin{prooftree}
	%	\AxiomC{}
	%	\UnaryInfC{$\hasKind{\Delta}{\textbf{CC}}{\type\to\type}$}
	%	\end{prooftree}
	%	
	%\begin{center}\fbox{$\hasType{\Delta}{\Gamma}{e}{\sigma}$}\end{center}
	%	
	%	\begin{prooftree}
	%	\AxiomC{}
	%	\UnaryInfC{$\hasType{\Delta}{\Gamma}{\newhandler}{\textbf{CC}\;(\Cue\;\sigma\;\sigma'})$}
	%	\end{prooftree}
	%
	%	\begin{prooftree}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e_p}{\Cue\;\sigma'\;\sigma}$}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e'}{\sigma'\to\textbf{CC}\;\sigma}$}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e}{\textbf{CC}\;\sigma}$}
	%	\TrinaryInfC{$\hasType{\Delta}{\Gamma}{\handle{e_p}{e'}{e}}{\textbf{CC}\;\sigma}$}
	%	\end{prooftree}
	%
	%	\begin{prooftree}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e_p}{\Cue\;\sigma'\;\sigma}$}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e}{\sigma'}$}
	%	\BinaryInfC{$\hasType{\Delta}{\Gamma}{\abort{e_p}{e}}{\textbf{CC}\;\sigma_{any}}$}
	%	\end{prooftree}
	%
	%	\begin{prooftree}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e_p}{\Cue\;\sigma''\;\sigma'}$}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e}{(\sigma''\to\textbf{CC}\;\sigma')\to\Subcont\;\sigma\;\sigma'\to\textbf{CC}\;\sigma}$}
	%	\BinaryInfC{$\hasType{\Delta}{\Gamma}{\capture{e_p}{e}}{\textbf{CC}\;\sigma}$}
	%	\end{prooftree}
	%
	%	\begin{prooftree}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e'}{\Subcont\;\sigma\;\sigma'}$}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e}{\sigma}$}
	%	\BinaryInfC{$\hasType{\Delta}{\Gamma}{\pushSubcont{e'}{e}}{\textbf{CC}\;\sigma'}$}
	%	\end{prooftree}
	%
	%	\begin{prooftree}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e_{in}}{\sigma'}$}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e}{\textbf{CC}\;\sigma}$}
	%	\BinaryInfC{$\hasType{\Delta}{\Gamma}{\onWind{e_{in}}{e}}{\textbf{CC}\;\sigma}$}
	%	\end{prooftree}
	%
	%	\begin{prooftree}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e_{out}}{\sigma'}$}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e}{\textbf{CC}\;\sigma}$}
	%	\BinaryInfC{$\hasType{\Delta}{\Gamma}{\onUnwind{e_{out}}{e}}{\textbf{CC}\;\sigma}$}
	%	\end{prooftree}
	%
	%	\begin{prooftree}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e_{out}}{\sigma'}$}
	%	\AxiomC{$\hasType{\Delta}{\Gamma}{e}{\textbf{CC}\;\sigma}$}
	%	\BinaryInfC{$\hasType{\Delta}{\Gamma}{\onAbort{e_{out}}{e}}{\textbf{CC}\;\sigma}$}
	%	\end{prooftree}
	%
	%\end{figure}
	%
	...
	%graduate to $\mathtt{runControl}:\forall a. (\forall r. \mathtt{Control}\;r\;a) \to a$
	%
	%\subsection{Combining Effects}
	%
	%monad transformers aren't the only way to combine effects, so lets just do this informally and mention a bunch of other ways to do it
	%
	%graduate to $\mathtt{runControlT} : \forall m,a. \mathtt{Monad}\;m \Rightarrow (\forall r. \mathtt{ControlT}\;r\;m\;a) \to m\;a$
...
%
%
%%$$\mathtt{Prompt}\;a \equiv \Cue\;a\;a$$
%%$$\mathtt{newPrompt} \equiv \mathtt{newHandler}\;\abs{x}{a}{x}$$
%$$\withSubcont{e}{e'} \equiv \capture{e}{x}{\abort{e}{(e'\;x)}}$$
%
%Note the free type variables, which must be bound in the context in which these operators appear.
%
%
%
%We can recover try/finally, scope(exit) and dynamic-wind with the following:
%
%
%$$\onExit{e}{e'} \equiv \onUnwind{e}{\onAbort{e}{e'}}$$
%$$\tryfin{e}{e'} \equiv \onExit{e'}{e}$$
%$$\dynamicWind{e_{in}}{e}{e_{out}} \equiv \onWind{e_{in}}{\onExit{e_{out}}{e}}$$
%
%The interesting part with onX will come when I add the control monad.
%
%TODO: there probably has to be a way for the $e_{in}$ (say, the result of opening a file) to give stuff (the file handle) to the body.
%
%
%
%\begin{prooftree}
%\AxiomC{$\hasType{\Delta}{\Gamma}{e}{\Cue\;\sigma\;\sigma}$}
%	\AxiomC{$\hasType{\Delta}{\Gamma}{e}{\Cue\;\sigma\;\sigma}$}
%		\AxiomC{$\hasType{\Delta}{\Gamma}{e'}{\Subcont\;\sigma'\;\sigma \to \sigma}$}
%	\UnaryInfC{$\hasType{\Delta}{\Gamma,x:\Subcont\;\sigma'\;\sigma}{e'\;x}{\sigma}$}
%	\BinaryInfC{$\hasType{\Delta}{\Gamma,x:\Subcont\;\sigma'\;\sigma}{\mathtt{raise}\;e\;(e'\;x)}{\sigma'}$}
%	\UnaryInfC{$\hasType{\Delta}{\Gamma}{\abs{x}{\Subcont\;\sigma'\;\sigma}{\mathtt{raise}\;e\;(e'\;x)}}{\Subcont\;\sigma'\;\sigma \to \sigma'}$}
%\BinaryInfC{$\hasType{\Delta}{\Gamma}{\mathtt{capture}\;e\;\abs{x}{\Subcont\;\sigma'\;\sigma}{\mathtt{raise}\;e\;(e'\;x)}}{\sigma'}$}
%\UnaryInfC{$\hasType{\Delta}{\Gamma}{\mathtt{withSubcont}\;e\;e'}{\sigma'}$}
%\end{prooftree}
%
%
%Problems in the above are that we should rule out stuff like the following as invalid:
%$$\onWind{(open\;file >> \abort{e}{e'})}{e''}$$
%$$\onExit{(\abort{e}{e'} >> close\;handle)}{e''}$$
%More generally, actions taken during wind-protection should be control-pure. This means that not only must we type control operators under some special monad, but we this monad must also be a monad transformer (in case there are still effects in the handlers, such as an IO action to open or close a file).
